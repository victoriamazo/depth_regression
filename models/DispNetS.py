...
class DispNetS(nn.Module):
    def __init__(self, FLAGS):
        ...
        self.conv1 = downsample_conv(FLAGS, input,              conv_planes[0], kernel_size=7)
        self.conv2 = downsample_conv(FLAGS, conv_planes[0], conv_planes[1], kernel_size=5)
        self.conv3 = downsample_conv(FLAGS, conv_planes[1], conv_planes[2])
        self.conv4 = downsample_conv(FLAGS, conv_planes[2], conv_planes[3])
        self.conv5 = downsample_conv(FLAGS, conv_planes[3], conv_planes[4])
        self.conv6 = downsample_conv(FLAGS, conv_planes[4], conv_planes[5])
        self.conv7 = downsample_conv(FLAGS, conv_planes[5], conv_planes[6])

        upconv_planes = [512, 512, 256, 128, 64, 32, 16]
        self.upconv7 = upconv_ReLU(FLAGS, conv_planes[6],   upconv_planes[0], output_padding=1)
        self.upconv6 = upconv_ReLU(FLAGS, upconv_planes[0], upconv_planes[1], output_padding=1)
        self.upconv5 = upconv_ReLU(FLAGS, upconv_planes[1], upconv_planes[2], output_padding=1)
        self.upconv4 = upconv_ReLU(FLAGS, upconv_planes[2], upconv_planes[3], output_padding=1)
        self.upconv3 = upconv_ReLU(FLAGS, upconv_planes[3], upconv_planes[4], output_padding=1)
        self.upconv2 = upconv_ReLU(FLAGS, upconv_planes[4], upconv_planes[5], output_padding=1)
        self.upconv1 = upconv_ReLU(FLAGS, upconv_planes[5], upconv_planes[6], output_padding=1)

        self.iconv7 = conv_ReLU(FLAGS, upconv_planes[0] + conv_planes[5], upconv_planes[0])
        self.iconv6 = conv_ReLU(FLAGS, upconv_planes[1] + conv_planes[4], upconv_planes[1])
        self.iconv5 = conv_ReLU(FLAGS, upconv_planes[2] + conv_planes[3], upconv_planes[2])
        self.iconv4 = conv_ReLU(FLAGS, upconv_planes[3] + conv_planes[2], upconv_planes[3])
        self.iconv3 = conv_ReLU(FLAGS, output + upconv_planes[4] + conv_planes[1], upconv_planes[4])
        self.iconv2 = conv_ReLU(FLAGS, output + upconv_planes[5] + conv_planes[0], upconv_planes[5])
        self.iconv1 = conv_ReLU(FLAGS, output + upconv_planes[6], upconv_planes[6])
